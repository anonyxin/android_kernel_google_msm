...
static bool aries_charger_mode;

static int __init aries_androidboot_mode_arg(char *options)
{
	if (!strcmp(options, "charger"))
		aries_charger_mode = true;
	return 0;
}
early_param("androidboot.mode", aries_androidboot_mode_arg);
...
static int aries_keyreset_fn(void)
{
	arm_pm_restart('h', NULL);
	return 1;
}

static struct keyreset_platform_data aries_reset_keys_pdata = {
	.keys_down	= {
		KEY_POWER,
		0,
	},
	.down_time_ms	= 1500,
	.reset_fn = aries_keyreset_fn,
};

struct platform_device aries_keyreset_device = {
	.name	= KEYRESET_NAME,
	.dev	= {
		.platform_data	= &aries_reset_keys_pdata,
	},
};
...
static void __init apq8064_common_init(void)
{
	...
	if (aries_charger_mode)
		platform_device_register(&aries_keyreset_device);
...
static void __init apq8064_aries_init(void)
{
	...
	pr_info("[DEBUG] uart_enable : %d", xiaomi_get_uart_mode() );
