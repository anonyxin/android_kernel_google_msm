...
#define ARIES_UART_TX_GPIO	10
#define ARIES_UART_RX_GPIO	11
...
static bool aries_uart_initialized = false;
...
{
	if (enable) {
		msm_gpiomux_write(ARIES_UART_TX_GPIO, GPIOMUX_ACTIVE,
				&gsbi4_uart, NULL);
		msm_gpiomux_write(ARIES_UART_TX_GPIO, GPIOMUX_SUSPENDED,
				&gsbi4_uart, NULL);
		msm_gpiomux_write(ARIES_UART_RX_GPIO, GPIOMUX_ACTIVE,
				&gsbi4_uart, NULL);
		msm_gpiomux_write(ARIES_UART_RX_GPIO, GPIOMUX_SUSPENDED,
				&gsbi4_uart, NULL);
	} else {
		msm_gpiomux_write(ARIES_UART_TX_GPIO, GPIOMUX_ACTIVE,
				&gsbi4_gpio, NULL);
...
static void earjack_debugger_set_console(void *dev)
{
	...
	if (earjack_debugger_detected(adev)) {
		if (!aries_uart_initialized) {
			aries_uart_initialized = true;
